...
parser.add_argument('--answer_module',
                    type=str,
                    default="feedforward",
...
parser.add_argument('--memory_hops',
                    type=int,
                    default=5,
...
parser.add_argument('--normalize_attention',
                    type=bool,
                    default=False,
...
def do_epoch(mode, epoch, skipped=0):
    ...
    for i in range(0, batches_per_epoch):
        ...
        if current_skip == 0:
            ...
            if (i % args.log_every == 0):
                cur_time = time.time()
                print(
                    "  %sing: %d.%d / %d \t loss: %.3f \t avg_loss: %.3f \t skipped: %d \t %s \t time: %.2fs"
                    % (mode, epoch, i * args.batch_size,
                       batches_per_epoch * args.batch_size, current_loss,
                       avg_loss / (i + 1), skipped, log, cur_time - prev_time))
                prev_time = cur_time
    ...
    print "accuracy: %.2f percent" % (accuracy * 100.0 / batches_per_epoch /
                                      args.batch_size)
...
if args.mode == 'train':
    ...
    for epoch in range(args.epochs):
        ...
        state_name = 'states/%s.epoch%d.test%.5f.state' % (
